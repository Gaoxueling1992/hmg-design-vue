  ...
  >
    <template v-if="activeCompObj.rules.length">
      ...
      <div v-for="(item, idx) in activeCompObj.rules" :key="idx">
        <div class="fontW500">规则{{idx + 1}}</div>
        ...
        执行动作：{{actionList[item.id]}} {{item.content}}
      </div>
    </template>
    <template v-if="editingRule">
      <div class="title marginT10 marginB5 fontW500 fontS16">新增规则(域值用${xx}代替)</div>
...
export default defineComponent({
  setup() {
    ...
    const saveRule = () => {
      ...
      console.log(ruleObj.value.type, ruleObj.value.value, ruleObj.value.type < 6 && !ruleObj.value.value)
      ...
      activeCompObj.value.rules.push({
        ...ruleObj.value,
        condition: {
          ...ruleObj.value.condition
        },
        action: {
          ...ruleObj.value.action
        }
      });
      ruleObj.value= {
        opportunity: 'init',
